...
void CoorDist() { //Покоординатно считаем квадраты разности 
	int countIns = 0;
	int countOut = 0;
	readMatrix();
#pragma omp parallel for reduction(+:countIns, countOut)
	for (int i = 0; i < m; i++)
	{
		for (int j = i + 1; j < m; j++)
		{
...
void InsCoorSum() { // Складываем квадраты разности из одного кластера и получаем расстояние
	CoorDist();
	int i = 0;
	while (insDist[i][0] != 0) {
		for (int j = 1; j <= n; j++) {
...
void OutCoorSum() { // Складываем квадраты разности из разных кластеров и получаем расстояние
	CoorDist();
	int i = 0;
	while (outDist[i][0] != 0) {
		for (int j = 2; j <= n + 1; j++) {
